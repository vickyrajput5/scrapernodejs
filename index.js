...
(async () => {
        ...
        try {
          ...
          if (titleElement && descriptionElement && stockElement) {
            ...
            const details = await page.evaluate(() => {
                ...
                // Extract content with the heading "Product description"
                const productDescriptionHeading = document.querySelector('section.md-col-6 h3:contains("Product description")');
                if (productDescriptionHeading) {
                    const productDescriptionContent = [];
                    let nextElement = productDescriptionHeading.nextElementSibling;
            
                    while (nextElement && nextElement.tagName.toLowerCase() === 'p') {
                        productDescriptionContent.push(nextElement.textContent.trim());
                        nextElement = nextElement.nextElementSibling;
                    }
            
                    result['Product Description'] = productDescriptionContent.join('\n');
                }
            ...
            console.log(`Product Description: ${details['Product Description'] || ''}`);
  ...
  ws.cell(1, 25).string('Product Description').style(headerStyle);
  ...
  allProducts.forEach((product) => {
    ...
    ws.cell(row, 25).string(product['Product Description'] || '');
    row++;
  });

  // Save the workbook
  const fileName = `ABFStoreProducts_1.xlsx`;
  wb.write(fileName);
